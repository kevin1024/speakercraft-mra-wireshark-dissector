...
function MRAPROTO.dissector (buffer, pinfo, tree)
    ...
    if (length:uint() > 1) then
	local result = buffer(offset, 1)
        offset = offset + 1
	subtree:add(f.result, result)
        local data = buffer (offset, length:uint() - 2)
	local data_tree = subtree:add (f.data, data)
        offset = offset + length:uint() - 2

	if (command:uint() == 3 and result:uint()==1) then
	...
	if (command:uint() == 0 and result:uint()==1) then
